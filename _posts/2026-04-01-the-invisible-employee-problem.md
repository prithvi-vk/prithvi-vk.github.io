...
The framing that clicked for me: *if the AI model is an employee, the orchestration layer is the company.*

The orchestration layer doesn't run AI models — it's a **control plane** that manages when agents wake up, what they work on, and what they spend. Think of it as a task tracker where the workers happen to be AI agents, complete with:
...
I stood up an instance for the CA firm with two agents:

**The Company:** A container that houses all agents, tasks, costs, and goals for the firm's operations.
...
**The platform runs locally** with an embedded database. No cloud dependencies for local development — one command brings up a full orchestration platform on localhost.

**Agents are configured with adapters** that define how the platform invokes their AI runtime. For the IT Notice Checker, I used a Claude Code integration that handles workspace resolution, session management, and permission configuration:
...
**I wired a Telegram bot** to the wake API. A simple long-polling Node.js server listens for `/check_notices` in a Telegram group and calls the wakeup endpoint. The agent spins up, does its work, and posts progress updates back to the same group.
...
All of this is visible in both the Telegram group and the orchestration dashboard. Two views of the same work — one for the team that operates via messaging, one for the stakeholder who wants a proper management interface.

## The Execution Loop

Every agent follows a protocol when it wakes up:
...
The firm's setup starts with one specialist agent. But the architecture is designed to grow. New agents — for GST compliance, audit preparation, client onboarding — can be hired through an approval flow. Each one gets the same visibility: org chart presence, run histories, cost tracking, and Telegram integration.
...
That infrastructure is what makes the difference.
