...
An AI agent — running on Claude with a browser under its control — that does steps 1 through 26 on its own. The CA's only job is to read the summary PDF that lands in their WhatsApp group, type 👍, and supply the OTP when it's time to file.
...
Once everything is entered, the agent clicks "Generate Summary," waits for the portal to crunch the numbers, downloads the consolidated summary PDF, and hands it off to the CA via WhatsApp Web — also driven by the same browser layer, also running headlessly, also handling the 47 ways WhatsApp's UI can subtly change between sessions.
