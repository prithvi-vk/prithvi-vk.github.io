...
keywords: "GSTR-1 automation India, GST portal automation, CA firm AI agent Kerala, automated GST return filing, GSTR-1 EVC filing automation, chartered accountant tax automation India"
---
...
Picture it: 1,009 service bills. A captcha that rotates every time you blink at it. A session that times out the second you make a cup of chai. And at the end of it all, an OTP from a phone you don't have on hand because the signatory is in another room.

Welcome to the last week of every month at a Chartered Accountant's office in Kerala.

Last week, I built an AI agent that does the entire thing — login to portal, fill every table, reconcile credit notes, generate the summary, share it with the CA on WhatsApp, wait for human approval, and then file the return via EVC. Real returns filed in production. ARNs in hand. Confirmation messages dispatched.

This is the story of how that came together.
...
- And other clients with the kind of "we'll figure out the data later" status that keeps CAs awake at night
...
Now do that for every client. In one week. While also doing actual accounting work.

## What I Built

An AI agent — running on Claude with a browser it drives itself — that does steps 1 through 26 on its own. The CA's only job is to read the summary PDF that lands in their WhatsApp group, type 👍, and supply the OTP when it's time to file.
...
A long-running browser session that the agent drives directly. It clicks, types, navigates, downloads files, takes screenshots when it needs to verify what it's seeing, and reads the page's accessibility tree (much faster than parsing HTML) to figure out where things are.
...
The agent handles it itself — no third-party captcha-solving service, and no human in the loop. A few seconds, every time.
...
It navigates each tile, fills each field, and saves each record — HSN summaries, documents issued, B2C consolidated, B2B invoices.
...
Once everything is entered, the agent clicks "Generate Summary," waits for the portal to crunch the numbers, downloads the consolidated summary PDF, and hands it off to the CA via WhatsApp Web — also driven by the agent, also handling the 47 ways WhatsApp's UI can subtly change between sessions.
...
One of the filings that has gone through was a mixed B2B + B2C return where the agent had to update Table 7, Table 12 on the B2C HSN tab, and recalculate the totals after the CA team sent updated Tally figures mid-flight. The agent handled the data update, regenerated the summary, downloaded the new PDF, and re-shared it to the WhatsApp group for re-approval. None of that required a human touching the browser.
...
The obvious win here is hours saved. A CA firm that spends four to six hours a month per client on GSTR-1 data entry, summary review, and filing logistics gets that time back. Across a full client book, that's easily a work-week a month — recovered.
...
More GSTR-1 filings to go this month — including a coaching institute filing that's going to be the most interesting of the lot because it touches almost every table in GSTR-1 (exports, exempt, advances, RCM, the works). The whole pipeline is now wired up to a **cron job** that runs on its own schedule, picks up clients in order, and works through the queue.

In parallel, the **income tax notice checker** running daily is about to scale up *significantly* — the next push adds hundreds of clients to the daily monitoring loop. The agent doesn't care. It runs the same sequence either way.
...
Over the last few weeks, I've automated income tax notice retrieval and GSTR-1 filing at a working CA firm. The conversations I've had with the partners — and with other CA firms who've heard about it through the grapevine — have made one thing very clear: **the demand is real, the pain is real, and the opportunity is real.**
...
I've now seen, firsthand, that this can be automated end-to-end. Real returns, real ARNs, real CAs approving on WhatsApp. The market opportunity is proven. The product-market fit is loud.
